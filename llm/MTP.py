    ...
    def generate(self, input_ids, max_length, **kwargs):
        ...
        with torch.no_grad():
            while seq.size(1) < max_length:
                ...
                logits = logits[:, -1, :]
                probs = torch.softmax(logits, dim=-1) # get最大概率的token
    ...
    for epoch in range(epochs):
        for step, batch in enumerate(dataloader):
            ...
            for index in range(0, config.predict_tokens_num - 1):
                ...
                mtp_head_output = mtp_head_output.reshape(-1,
                                                          model.main_model.config.vocab_size)  # [batch_size * seq_len, vocab_size]
            ...
            main_loss = F.cross_entropy(main_head_output[:, :-1].reshape(-1, model.main_model.config.vocab_size),
            ...
            if (steps + 1) % print_step == 0:
                ...
                print(
                    f"Epoch {epoch + 1}], Step {steps + 1}, main_loss: {main_loss.item():.4f}, mtp_loss: {mtp_loss.item():.4f}")
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        q = self.tokenizer.apply_chat_template([{"role": "user", "content": user}], tokenize=False,
...
#     train(config, model, dataloader, optimizer, writer, device='cuda', epochs=10, print_step=10, save_step=500,
#           save_path='mtp')
