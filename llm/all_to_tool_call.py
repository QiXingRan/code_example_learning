...
def generate_text(base_url: str, model: str, messages: List, max_tokens: int, temperature: float, api_key: str,
                  tools=None):
...
async def create_chat_completion(request: ChatCompletionRequest):
    if request.tools: # request.tools：可调用工具列表
        TOOL_EAXMPLE = "You will receive a JSON string containing a list of callable tools. Please parse this JSON string and return a JSON object containing the tool name and tool parameters." # 向大模型获取工具调用的Json字符
        ...
        response = generate_text(args.no_tool_call_base_url, args.no_tool_call_model_name, messages, request.max_tokens,
                                 request.temperature, args.no_tool_call_api_key)
        ...
        response = generate_text(args.tool_call_base_url, args.tool_call_model_name, messages, request.max_tokens,
                                 request.temperature, args.tool_call_api_key, tools=request.tools)

    else:
        response = generate_text(args.no_tool_call_base_url, args.no_tool_call_model_name, request.messages,
                                 request.max_tokens, request.temperature, args.no_tool_call_api_key)
