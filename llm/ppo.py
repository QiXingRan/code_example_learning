...
# critic（评论家）模型，用于预测每一步（生成token）的动作产生的收益，使用action模型进行初始化，并外加一个回归头，输出shape为：(batch_size, seq_len， 1)
class Critic(nn.Module):
    def __init__(self, base_model):
        super().__init__()
        self.base_model = base_model
        self.base_model.eval() # 正常是参与模型更新的，现在只是demo所以冻结梯度
        self.value_head = nn.Linear(base_model.config.hidden_size, 1) # 回归头，输出1维度代表token的奖励值
...
def get_advantages_and_returns(
        ...
        gamma: float,
        lambd: float):
    lastgaelam = 0 # 最后一个时刻的未来优势和未来收益为0
    advantages_reversed = []
...
def generate_experiences(samples_list):
    ...
    for samples in samples_list: # 取样本
        seqs = samples.seqs
